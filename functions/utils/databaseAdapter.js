...
export function createDatabaseAdapter(env) {
    // 检查是否配置了数据库
    if (env.img_kvrocks && typeof env.img_kvrocks.get === 'function') {
        // 使用KVROCKS存储
        ...
        console.error('No database configured. Please configure either KVROCKS (env.img_kvrocks), KV (env.img_url) or D1 (env.img_d1).');
        return null;
    }
}
...
class KVROCKSAdapter {
    constructor(kvrocks) {
        this.kvrocks = kvrocks;
    }

    // 直接代理到KVROCKS的方法
    async put(key, value, options) {
        options = options || {};
        return await this.kvrocks.put(key, value, options);
    }

    async get(key, options) {
        options = options || {};
        return await this.kvrocks.get(key, options);
    }

    async getWithMetadata(key, options) {
        options = options || {};
        return await this.kvrocks.getWithMetadata(key, options);
    }

    async delete(key, options) {
        options = options || {};
        return await this.kvrocks.delete(key, options);
    }

    async list(options) {
        options = options || {};
        return await this.kvrocks.list(options);
    }
...
export function getDatabase(env) {
    ...
    if (!adapter) {
        throw new Error('Database not configured. Please configure KVROCKS database (env.img_kvrocks), KV storage (env.img_url) or D1 database (env.img_d1).');
    }
    return adapter;
}
...
export function checkDatabaseConfig(env) {
    ...
    var hasKVROCKS = env.img_kvrocks && typeof env.img_kvrocks.get === 'function';
    ...
    return {
        ...
        usingD1: hasD1,
        usingKV: !hasD1 && !hasKVROCKS && hasKV,
        usingKVROCKS: hasKVROCKS,
        configured: hasD1 || hasKV || hasKVROCKS
    };
}
